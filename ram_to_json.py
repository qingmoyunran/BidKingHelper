...
# ========== 新增：UTF-16LE JSON 日志解析 ==========
def extract_utf16le_json(pm: pymem.Pymem, anchor_addr: int, search_radius: int = 2048) -> Optional[Tuple[str, int, int]]:
    """
    以 anchor_addr 为中心，在前后 search_radius 字节内搜索 UTF-16LE 编码的 JSON 对象。
    ...
    start_pos = -1
    end_pos = -1
    # 从左到右找第一个 '{' 的 UTF-16LE 表示
    for i in range(len(data) - 1):
        if data[i] == 0x7b and data[i+1] == 0x00:
            start_pos = i
            break
    if start_pos == -1:
        return None
    # 从 start_pos 往后找匹配的 '}'，注意栈匹配（简单起见，找第一个后面的 '}\x00' 且中间没有不匹配的括号？简化：找最近的 '}'）
    # 更可靠：找到 start_pos 后，扫描直到遇到 '}\x00'，并确保大括号数量平衡（简单实现）
    balance = 0
    for i in range(start_pos, len(data) - 1):
        if data[i] == 0x7b and data[i+1] == 0x00:
            balance += 1
        elif data[i] == 0x7d and data[i+1] == 0x00:
            balance -= 1
            if balance == 0:
                end_pos = i
                break
    if end_pos == -1:
        return None
    # 提取字节并解码
    json_bytes = data[start_pos:end_pos+2]  # 包含最后的 '}\x00'
    try:
        json_str = json_bytes.decode('utf-16le')
    except UnicodeDecodeError:
        return None
    return (json_str, start + start_pos, start + end_pos + 2)

def save_json_log(room_id: str, scan_idx: int, json_str: str, addr_start: int, addr_end: int):
    """将解析出的 JSON 日志保存为可读文件"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # 将房间号中的冒号替换为下划线，避免非法字符
    safe_room_id = room_id.replace(':', '_')
    filename = os.path.join(OUTPUT_DIR, f"json_log_{safe_room_id}_{scan_idx:03d}_{timestamp}.txt")
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(f"房间号: {room_id}\n")
            f.write(f"扫描次数: {scan_idx}\n")
            f.write(f"内存地址范围: 0x{addr_start:X} - 0x{addr_end:X}\n")
            ...
            try:
                ...
                # 如果不是有效 JSON，直接输出字符串
                f.write(json_str)
        print(f"JSON 日志已保存到 {filename}")
    except Exception as e:
        print(f"保存 JSON 日志失败: {e}")

# ========== 修改 scanning_worker 函数 ==========
        ...
        if room_suffix:
            try:
                ...
                if suffix_matches:
                    ...
                    # 新增：对每个匹配的后缀地址，尝试提取附近的 JSON 日志
                    for addr, before, after in suffix_matches:
                        json_info = extract_utf16le_json(pm, addr, 9216)
                        if json_info:
                            json_str, start_addr, end_addr = json_info
                            save_json_log(room_id, scan_counter, json_str, start_addr, end_addr)
                else:
                    print("未找到匹配的房间号后缀。")
            except Exception as e:
                print(f"房间号后缀扫描出错: {e}")
